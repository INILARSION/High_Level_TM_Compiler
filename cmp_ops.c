...
struct delta_group *bigger_operation(struct program *program, int start_state, int subsequent_state_true, int subsequent_state_false, int band_1, int band_2) {
    program->state_count += 6;
    program->state_names = realloc(program->state_names, program->state_count * sizeof(char *));

    program->state_names[program->state_count-6] = calloc(strlen(program->state_names[start_state]) + 5, sizeof(char));
    sprintf(program->state_names[program->state_count-6], "%sbigs",program->state_names[start_state]);
    int end_sub_op = program->state_count-6;

    program->state_names[program->state_count-5] = calloc(strlen(program->state_names[start_state]) + 5, sizeof(char));
    sprintf(program->state_names[program->state_count-5], "%sbigf",program->state_names[start_state]);
    int go_to_end = program->state_count-5;

    program->state_names[program->state_count-4] = calloc(strlen(program->state_names[start_state]) + 8, sizeof(char));
    sprintf(program->state_names[program->state_count-4], "%sbigbdec",program->state_names[start_state]);
    int test_bigger = program->state_count-4;

    program->state_names[program->state_count-3] = calloc(strlen(program->state_names[start_state]) + 6, sizeof(char));
    sprintf(program->state_names[program->state_count-3], "%sbigbt",program->state_names[start_state]);
    int bigger_maybe_true = program->state_count-3;

    program->state_names[program->state_count-2] = calloc(strlen(program->state_names[start_state]) + 7, sizeof(char));
    sprintf(program->state_names[program->state_count-2], "%sbigbtt",program->state_names[start_state]);
    int bigger_true = program->state_count-2;

    program->state_names[program->state_count-1] = calloc(strlen(program->state_names[start_state]) + 6, sizeof(char));
    sprintf(program->state_names[program->state_count-1], "%sbigbf",program->state_names[start_state]);
    int bigger_false = program->state_count-1;

    struct delta_group *sub_deltas = sub_operation(program, start_state, end_sub_op, band_1, band_2, program->head_count - 1);
    ...
    // start compare
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->state_name = end_sub_op;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->subsequent_state = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->write_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->movements = movement_right;

    // go to end
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 1] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 1]->state_name = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 1]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 1]->subsequent_state = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 1]->write_symbols = symbol_0;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 2]->state_name = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 2]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 2]->subsequent_state = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 2]->write_symbols = symbol_1;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->state_name = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->subsequent_state = test_bigger;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->write_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->movements = movement_left;

    // decide to true
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->state_name = test_bigger;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->subsequent_state = bigger_maybe_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->write_symbols = symbol_0;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->state_name = test_bigger;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->subsequent_state = bigger_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->write_symbols = symbol_1;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->state_name = bigger_maybe_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->subsequent_state = bigger_maybe_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->write_symbols = symbol_0;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->state_name = bigger_maybe_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->subsequent_state = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->write_symbols = symbol_1;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->state_name = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->subsequent_state = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->write_symbols = symbol_0;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->state_name = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->subsequent_state = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->write_symbols = symbol_1;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->state_name = bigger_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->subsequent_state = bigger_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->write_symbols = symbol_0;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->state_name = bigger_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->subsequent_state = bigger_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->write_symbols = symbol_1;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 12]->state_name = bigger_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 12]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 12]->subsequent_state = subsequent_state_true;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 13]->state_name = bigger_maybe_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 13]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 13]->subsequent_state = subsequent_state_false;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 14]->state_name = bigger_false;
...
struct delta_group *bigger_equal_operation(struct program *program, int start_state, int subsequent_state_true, int subsequent_state_false, int band_1, int band_2) {
    ...
    program->state_names[program->state_count-5] = calloc(strlen(program->state_names[start_state]) + 8, sizeof(char));
    sprintf(program->state_names[program->state_count-5], "%sbigequs",program->state_names[start_state]);
    int sub_end = program->state_count-5;

    program->state_names[program->state_count-4] = calloc(strlen(program->state_names[start_state]) + 8, sizeof(char));
    sprintf(program->state_names[program->state_count-4], "%sbigequf",program->state_names[start_state]);
    int go_to_end = program->state_count-4;

    program->state_names[program->state_count-3] = calloc(strlen(program->state_names[start_state]) + 11, sizeof(char));
    sprintf(program->state_names[program->state_count-3], "%sbigequbdec",program->state_names[start_state]);
    int test_bigger_equ = program->state_count-3;

    program->state_names[program->state_count-2] = calloc(strlen(program->state_names[start_state]) + 9, sizeof(char));
    sprintf(program->state_names[program->state_count-2], "%sbigequb1",program->state_names[start_state]);
    int bigger_equ_true = program->state_count-2;

    program->state_names[program->state_count-1] = calloc(strlen(program->state_names[start_state]) + 9, sizeof(char));
    sprintf(program->state_names[program->state_count-1], "%sbigequb2",program->state_names[start_state]);
    int bigger_equ_false = program->state_count-1;
    ...
    // start compare
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->state_name = sub_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->subsequent_state = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->write_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 0]->movements = movement_right;
    ...
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->state_name = go_to_end;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->subsequent_state = test_bigger_equ;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->write_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 3]->movements = movement_left;

    // decide
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->state_name = test_bigger_equ;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->subsequent_state = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->write_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 4]->movements = movement_left;

    // decide
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->state_name = test_bigger_equ;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->subsequent_state = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->write_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 5]->movements = movement_left;

    // go back true
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->state_name = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->subsequent_state = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->write_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 6]->movements = movement_left;

    // go back true
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->state_name = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->subsequent_state = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->write_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 7]->movements = movement_left;

    // go back false
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->state_name = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->read_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->subsequent_state = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->write_symbols = symbol_0;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 8]->movements = movement_left;

    // go back false
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->state_name = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->read_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->subsequent_state = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->write_symbols = symbol_1;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 9]->movements = movement_left;

    // go back true stop
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->state_name = bigger_equ_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->subsequent_state = subsequent_state_true;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->write_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 10]->movements = movement_stop;

    // go back false stop
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11] = malloc(sizeof(struct delta));
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->state_name = bigger_equ_false;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->read_symbols = symbol_blank;
    bigger_equal_deltas->deltas[sub_deltas->delta_count + 11]->subsequent_state = subsequent_state_false;
